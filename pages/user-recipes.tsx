  ...
  return (
    <Layout>
      <div className="flex flex-col items-center justify-center border border-gray-300 overflow-auto">
        {savedRecipes.length > 0 ? (savedRecipes.map((item: Recipe, i: number) => {
          return (
            <details key={i}>
              <summary className="cursor-pointer">{item.title}</summary>
              <h3 className="whitespace-pre-wrap">{item.content}</h3>
              <button onClick={() => deleteRecipeFromDb(i)}>Test Delete</button>
            </details>
          )
        }))
          :
          <h2>No recipes saved! Generate a recipe and click save for it to appear</h2>
        }
      </div>
    </Layout>
  )
