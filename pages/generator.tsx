...
export default function GeneratorPage() {
  ...
  async function getSuggestedIngredients() {
    if (input) {
      console.log(
        `${process.env.NEXT_PUBLIC_URL}/api/spoonacular?input=${input}`,
      );
  ...
  async function fetchOpenApi() {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/openai`, {
        ...
        body: JSON.stringify({ ingredients: ingredients, diet: selectedDiet }),
      });
      ...
      <div className="md:flex-row flex flex-col justify-between text-m sm:text-md sm:border sm:border-grey-300 md:border-none md:text-lg">
        ...
        >
          ...
          >
            <div
              id="search-wrapper"
              className="self-start flex items-start justify-between flex-col h-[20%] w-[100%] md:h-[90%] md:w-[40%]"
            >
              <input
                type="text"
            ...
            <div
              id="ingredients"
              className="flex items-start justify-between flex-col h-[20%] w-[100%] md:h-[90%] md:w-[50%]"
            >
              <h3 className=" text-xl xl:text-2xl font-bold mb-4 md:mb-8 md:mt-0">
                Selected Items:
              </h3>
            ...
            <div className="self-center md:self-center grid grid-rows-2 grid-cols-2 gap-5 h-[30%] md:flex md:flex-col justify-center md:h-[100%]">
              <button
                className={`py-2 px-4 h-fit rounded-lg border border-gray-300 ${
                  selectedDiet === "vegan" ? "bg-blue-500 text-white" : ""
                }`}
                onClick={() => {
              ...
              <button
                className={`py-2 px-4 h-fit rounded-lg border border-gray-300 ${
                  selectedDiet === "vegetarian" ? "bg-blue-500 text-white" : ""
                }`}
                onClick={() => {
              ...
              <button
                className={`py-2 px-4 h-fit rounded-lg border border-gray-300 ${
                  selectedDiet === "pescetarian" ? "bg-blue-500 text-white" : ""
                }`}
                onClick={() => {
              ...
              <button
                className={`py-2 px-4 h-fit rounded-lg border border-gray-300 ${
                  selectedDiet === "keto" ? "bg-blue-500 text-white" : ""
                }`}
                onClick={() => {
          ...
          <div className="grid grid-rows-2 grid-cols-2 gap-5 md:grid-cols-4 md:grid-rows-1 md:gap-5 md:w-[90%] md:h-[15%] rounded-lg">
            ...
            <ReactToPrint
              trigger={() => (
                <button className="py-2 px-4 rounded-lg border border-gray-300 text-l">
                  🖨️ <br></br> Print
                </button>
              )}
